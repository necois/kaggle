...
class DataProcessor:
    ...
    def compute_features(self, df: pd.DataFrame) -> List[str]:
        return list(
            set(df.drop(self.id_column, axis=1).columns) - set(self.raw_target_columns)
        )

    @staticmethod
    def compute_symbolic_regression_features(
        ...
        return df.assign(
            feature1=np.sqrt((np.sqrt(X3 - np.log(X1)) + X3) - np.log(X1)),
            feature2=np.sqrt(np.sqrt(X14) - np.sqrt(X3)),
        )

    def process_targets(self, df: pd.DataFrame) -> pd.DataFrame:
        ids_to_remove = (
            df.loc[lambda df: df.loc[:, self.raw_target_columns].sum(axis=1) > 1]
            .get(self.id_column)
            .values
        )

        print(
            "Removed rows", len(ids_to_remove), 100 * len(ids_to_remove) / df.shape[0]
        )

        df = df.loc[lambda df: ~df.get(self.id_column).isin(ids_to_remove)].assign(
            target=None
        )
        ...
        for raw_target in self.raw_target_columns:
            dfs.append(
                df.loc[lambda df: df.get(raw_target) == 1].assign(
                    **{self.DEFAULT_TARGET_NAME: raw_target}
                )
            )
    ...
    def run(self):
        ...
        self.train_df = self.compute_symbolic_regression_features(
            train_df, self.raw_features
        )
        self.test_df = self.compute_symbolic_regression_features(
            test_df, self.raw_features
        )
        ...
        self.y_label = self.label_encoder.fit_transform(
            train_df.loc[:, [self.DEFAULT_TARGET_NAME]]
        )
        self.class_weight_dict, self.sample_weights = (
            self.compute_classs_and_sample_weights(self.y_label)
        )

        self.labels = self.label_encoder.inverse_transform(
            list(self.class_weight_dict.keys())
        )
        self.class_label_dict = dict(zip(self.class_weight_dict.keys(), self.labels))

        self.X_train, self.X_test, self.y_train, self.y_test = train_test_split(
            train_df.loc[:, self.features],
            self.y_label,
