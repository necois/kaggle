...
def correlation_matrix(
    ...
    if method not in ALLOWED_CORRELATION_METHODS:
        raise ValueError(
            f"method must be one of the following: {', '.join(ALLOWED_CORRELATION_METHODS)}"
        )
    ...
    filtered_correlation = correlation_matrix.where(
        (abs(correlation_matrix) > threshold) & (correlation_matrix != 1.0)
    )
    pairs = filtered_correlation.unstack().dropna()
    unique_pairs = pairs.loc[
...
def compare_distributions_and_stats(df1: pd.DataFrame, df2: pd.DataFrame, feature: str):
    ...
    plt.title(
        f"{feature}: Skewness (Train, Test) = ({train_skewness}, {test_skewness}) & Kurtosis (Train, Test) = ({train_kurtosis}, {test_kurtosis})"
    )
